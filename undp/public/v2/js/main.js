...
var  SSAConflict = (function(){
  ...
  var filterEvents = function(){
     ...
     $(".filters .years .year .dd").on("click",function(){
        $(".filters .years .year .hidden").hide();
        $(this).parent().find(".hidden").toggle();
     });
     $(".filters .years .year .hidden .label").on("click",function(){   
        var orig = timePeriod.from + "-" + timePeriod.to
        $(this).parent().toggle();
        var yr = parseInt($(this).text());
        if($(this).parent().parent().hasClass("from")){
          timePeriod.from = yr < timePeriod.to ? yr : timePeriod.from;
          $(this).parent().parent().find("> .label").html(timePeriod.from);
        }else{
          timePeriod.to = yr > timePeriod.from ? yr : timePeriod.to;
