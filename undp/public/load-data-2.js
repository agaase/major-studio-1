...
function drawConflictsPerCountry() {
    ...
    for(var conflictInYr in conflictsPerYrPerCountry){
      ...
      for(var country in yearConflicts){
        ...
        if(!perCountryData["SSA"][conflictInYr]){
            perCountryData["SSA"][conflictInYr] = yearConflicts[country];
        }else{
            perCountryData["SSA"][conflictInYr] += yearConflicts[country];
        }
      }
    }
    var cellWidth = Math.floor((windowWidth-120)/countries);
    var ctCountry = 0, cellHeight = 25, marginTop = 90;
    resizeCanvas(windowWidth, cellHeight*countries+100);

    textStyle(BOLD);
    textSize("24");
    text("SSA Conflicts (Over time)",20,40);
    textSize("14");
    text(startingYear,100,60,10,20);
    text(startingYear+countries-1,windowWidth-60,60,10,20);
    ...
    for(var country in perCountryData){
      ...
      text(country, 20, marginTop+cellHeight*ctCountry,80,cellHeight);
      for(var z=0;z<=countries;z++){
          var intensity = perCountryData[country][startingYear + z];
          noStroke();
          if(!intensity){
              noFill();
              rect(100+z*cellWidth, marginTop+cellHeight*ctCountry, cellWidth, cellHeight);
          }else{
      ...
      stroke("#9B9B9B");
      line(20, marginTop+cellHeight*(ctCountry+1), windowWidth-30, marginTop+cellHeight*(ctCountry+1));
      ctCountry++;
    }
}
